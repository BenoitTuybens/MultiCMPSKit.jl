...
function addkronecker!(O::AbstractMatrix, A::AbstractMatrix, B::AbstractMatrix,
                       α=one(eltype(O)))
    mA, nA = size(A)
    mB, nB = size(B)
    @assert size(O) == (mA * mB, nA * nB)
    O2 = reshape(O, (mA, mB, nA, nB))
    @inbounds for l in 1:nB
        for k in 1:nA
            for j in 1:mB
                @simd for i in 1:mA
...
function addkronecker!(O::AbstractMatrix, A::AbstractMatrix, B::UniformScaling,
                       α=one(eltype(O)))
    mA, nA = size(A)
...
function addkronecker!(O::AbstractMatrix, A::UniformScaling, B::AbstractMatrix,
                       α=one(eltype(O)))
    mB, nB = size(B)
