...
function Base.:*(α::ScalarFunction, op::SumOfLocalTerms)
    return SumOfLocalTerms(α .* op.coefficients, op.operators)
end
...
Base.:-(op1::LocalOperator, op2::LocalOperator) = SumOfLocalTerms((1, -1), (op1, op2))
function Base.:+(op1::SumOfLocalTerms, op2::LocalOperator)
    return SumOfLocalTerms((op1.coefficients..., 1), (op1.operators..., op2))
end
function Base.:-(op1::SumOfLocalTerms, op2::LocalOperator)
    return SumOfLocalTerms((op1.coefficients..., -1), (op1.operators..., op2))
end
function Base.:+(op1::LocalOperator, op2::SumOfLocalTerms)
    return SumOfLocalTerms((1, op2.coefficients...), (op1, op2.operators...))
end
Base.:-(op1::LocalOperator, op2::SumOfLocalTerms) = +(op1, -(op2))
function Base.:+(op1::SumOfLocalTerms, op2::SumOfLocalTerms)
    return SumOfLocalTerms((op1.coefficients..., op2.coefficients...),
                           (op1.operators..., op2.operators...))
end
Base.:-(op1::SumOfLocalTerms, op2::SumOfLocalTerms) = +(op1, -(op2))

function Base.:*(op1::SumOfLocalTerms, op2::LocalOperator)
    return SumOfLocalTerms(op1.coefficients, op1.operators .* (op2,))
end
function Base.:*(op1::LocalOperator, op2::SumOfLocalTerms)
    return SumOfLocalTerms(op2.coefficients, (op1,) .* op2.operators)
end

function Base.:*(op1::SumOfLocalTerms{1}, op2::SumOfLocalTerms)
    return op1.coefficients[1] * (op1.operators[1] * op2)
end
...
function Base.adjoint(op::SumOfLocalTerms)
    return SumOfLocalTerms(adjoint.(op.coefficients), adjoint.(op.operators))
end
