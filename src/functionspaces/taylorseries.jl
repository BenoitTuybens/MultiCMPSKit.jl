...
mutable struct TaylorSeries{T,S<:Real} <: FunctionSeries{T}
    coeffs::Vector{T}
    offset::S
    function TaylorSeries(coeffs::Vector{T}, offset=0.0) where {T}
        @assert length(coeffs) >= 1
        S = typeof(offset)
        return new{T,S}(coeffs, offset)
    end
end
...
function truncate!(f::TaylorSeries; Kmax::Integer=degree(f), tol::Real=0, dx::Real=1.0)
...
function Base.similar(f::TaylorSeries, ::Type{T}=scalartype(f)) where {T}
    return TaylorSeries([zero(T) * f[0]], offset(f))
end
...
function Base.:*(f::TaylorSeries, a::Const)
    return TaylorSeries([c * a for c in coefficients(f)], offset(f))
end
function Base.:*(a::Const, f::TaylorSeries)
    return TaylorSeries([a * c for c in coefficients(f)], offset(f))
end
function Base.:/(f::TaylorSeries, a::Const)
    return TaylorSeries([c / a for c in coefficients(f)], offset(f))
end
...
function Base.:+(f1::TaylorSeries, f2::TaylorSeries)
    ...
    coeffs = [f1[j] + f2[j] for j in 0:K]
    for j in (K + 1):degree(f1)
        push!(coeffs, copy(f1[j]))
    end
    for j in (K + 1):degree(f2)
...
LinearAlgebra.axpy!(α::Number, fx::TaylorSeries, fy::TaylorSeries) = truncadd!(fy, fx, α)
function LinearAlgebra.axpby!(α::Number, fx::TaylorSeries, β::Number, fy::TaylorSeries)
    return truncadd!(fy, fx, α, β)
end
function LinearAlgebra.mul!(fy::TaylorSeries, s::Number, fx::TaylorSeries, α=true, β=false)
    return truncmul!(fy, s, fx, α, β)
end
function LinearAlgebra.mul!(fy::TaylorSeries, fx::TaylorSeries, s::Number, α=true, β=false)
    return truncmul!(fy, fx, s, α, β)
end
function LinearAlgebra.mul!(f::TaylorSeries, f1::TaylorSeries, f2::TaylorSeries,
                            α=true, β=false)
...
function truncmul!(fdst::TaylorSeries, α₁::Number, fsrc::TaylorSeries, α₂=true, β=false;
                   kwargs...)
    return truncadd!(fdst, fsrc, α₁ * α₂, β; kwargs...)
end
function truncmul!(fdst::TaylorSeries, fsrc::TaylorSeries, α₁::Number, α₂=true, β=false;
                   kwargs...)
    return truncadd!(fdst, fsrc, α₁ * α₂, β; kwargs...)
end
...
function differentiate(f::TaylorSeries)
    if degree(f) >= 1
        return TaylorSeries([k * f[k] for k in 1:degree(f)], offset(f))
    else
        return zero(f)
    end
end
...
function map_sesquilinear(φ, f₁::TaylorSeries, f₂::TaylorSeries; kwargs...)
    return map_bilinear(φ, f₁, f₂; kwargs...)
end
...
function _map_bilinear(φ, f₁::TaylorSeries, f₂::TaylorSeries;
    ...
    for k in 1:K
        ...
        for l in max(0, k - degree(f₁)):min(k, degree(f₂))
...
function fit(f, ::Type{TaylorSeries}, domain::Tuple{Real,Real};
    ...
    return TaylorSeries(pinv(coeffs) * f.(x), offset)
end
...
function Base.inv(t::TaylorSeries; tol=1e-10, Kmax=10 * degree(t))
    ...
    K = degree(t)
    for k in 1:Kmax
        invtk = -t0inv * sum(coeffs[i + 1] * t[k - i] for i in max(0, k - K):(k - 1))
