function groundstate(H::LocalHamiltonian, Ψ₀::FiniteCMPS;
    ...
    function retract(x, g, α)
        ...
        HL, HR, ρL, ρR, hL, hR, E, e, infoHL, infoHR, infoρL, infoρR = environments!(H, Ψ;
                                                                                     Kmax=Kmax,
                                                                                     tol=tol)
    ...
    x = let (Q₀, R₀s, vL₀, vR₀) = Ψ₀
        ...
        Rs = map(R₀s) do R₀
            return PiecewiseLinear(collect(R₀.nodes), copy.(R₀.values))
        end
        vL = vL₀ / norm(vL₀)
        vR = vR₀ / norm(vR₀)
        Ψ = FiniteCMPS(Q, Rs, vL, vR)
...
function groundstate2(H::LocalHamiltonian, Ψ₀::FiniteCMPS;
                      optalg::OptimKit.OptimizationAlgorithm=
                      LBFGS(30; verbosity=2, gradtol=1e-3),
                      boundaries=:fixed,
                      left_boundary=boundaries,
    ...
    function precondition(x, g)
        Ψ, HL, HR, ρL, ρR, hL, hR, E, e = x
        g′, info = linsolve(g, g, CG(; verbosity=0, maxiter=5, tol=1e-9)) do Φ
            dQ, dRs = CMPSKit.metric(Φ, (Ψ, ρL, ρR))
            return axpy!(1e-2, Φ, FiniteCMPSTangent(Ψ, dQ, dRs, zero(Ψ.vL), zero(Ψ.vR)))
        end
        return g′
    end
